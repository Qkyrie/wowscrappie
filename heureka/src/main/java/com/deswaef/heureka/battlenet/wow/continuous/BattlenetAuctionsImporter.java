...
@Component
public class BattlenetAuctionsImporter {
    ...
    protected void importAuctions(Realm realm) {
        try {
    ...
    private void updateSnapshotConfiguration(Realm realm, AuctionResponse.AuctionResponseFile auctionSnapshot) {
        ...
        if (byRealmId.isPresent()) {
            ...
            generatedSnapshotRealmConfiguration.setLastUpdate(new Date(auctionSnapshot.getLastModified()));
    ...
    private void save(List<AuctionHouseSnapshot> converted) {
        converted
                ...
                .forEach(convert -> {
                    try {
                        auctionHouseSnapshotRepository.save(convert);
                    } catch (Exception exception) {
                        try {
                            auctionHouseSnapshotRepository.save(
                                    convert.setItem(getItemFromWowhead(convert.getItem().getId()))
                            );
                        } catch (Exception e) {
                            logger.info(String.format("Item not found: %d", convert.getItem().getId()));
                        }
                    }
                });
    }
    ...
    private Item getItemFromWowhead(Long itemId) {
    ...
    private boolean needsUpdate(Realm realm, List<AuctionResponse.AuctionResponseFile> files) {
        ...
        return !(snapshotHistory.isPresent() && (files.get(0).getLastModified() == snapshotHistory.get().getLastUpdate().getTime() || !snapshotHistory.get().isNeedsUpdate()));
    }
