    ...
    History.Adapter.bind(window, 'statechange', function () { // Note: We are using statechange instead of popstate
        ...
        if (State.data.configId !== undefined) {
            ...
            if (configtype !== undefined && configtype !== 0 && configtype !== null && bossid !== undefined && bossid !== 0 && bossid !== null) {
                $.get("/raids/3/bosses/" + State.data.bossId + "/" + State.data.configType, function (data) {
...
if (configtype !== 0 && configtype !== null && bossid !== 0 && bossid !== null) {
    $.get("/raids/3/bosses/" + bossid + "/" + configtype, function (data) {
        $("#resultPane").html(data);
        $('#myTable')
            .on('init.dt', function () {
                if (slidingHeader.manualPreference !== true) {
...
$(".btn-macro").click(function () {
    ...
    History.pushState(
        ...
        "Hellfire Citadel Macros - WowScrappie", "?bossid=" + bossId + '&configtype=macro'
    );
});
...
$(".btn-wa").click(function () {
    ...
    History.pushState(
        ...
        "Hellfire Citadel Weakauras - WowScrappie", "?bossid=" + bossId + '&configtype=weakaura'
    );
});
...
$(".btn-tmw").click(function () {
    ...
    History.pushState(
        ...
        "Hellfire Citadel TellMeWhens - WowScrappie", "?bossid=" + bossId + '&configtype=tellmewhen'
    );
});